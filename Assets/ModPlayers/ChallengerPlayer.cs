...
public class ChallengerPlayer : ModPlayer
{
        ...
        switch (type)
        {
            ...
            case ChallengerOrbType.Yellow:
                ...
                Projectile.NewProjectile(new EntitySource_OnHit(Main.player[projectile.owner], victim),
                    victim.position.X, victim.position.Y, 0f, 0f, ProjectileID.VampireHeal, 0, 0f,
    ...
    public override void SendClientChanges(ModPlayer clientPlayer)
    {
        ...
        if (Math.Abs(ChallengerScore - clone.ChallengerScore) > 0.01f)
    ...
    private void ChallengerTick()
    {
        ...
        bool challengerWeaponHeld = Player.HeldItem.prefix == ModContent.PrefixType<PrefixChallenger>();
    ...
    public override void ModifyShootStats(Item item, ref Vector2 position, ref Vector2 velocity, ref int type,
        ...
        ref float knockback)
    {
        if (ChallengerModeActive)
        {
            velocity *= GetChallengerScalar(ChallengerProjVelMultiplier);
        }
    }
