...
namespace EquipmentEvolved.Assets.Projectiles;
...
public class PhantomArmorProjectile : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        {
            if (phantom.heldProj >= 0 && Main.projectile[phantom.heldProj].active && Main.projectile[phantom.heldProj].owner == owner.whoAmI)
            {
                Main.projectile[phantom.heldProj].Kill();
            }
        ...
        if (heldShoot > 0 && heldShoot < phantom.ownedProjectileCounts.Length)
        {
            oldProjCount = phantom.ownedProjectileCounts[heldShoot];
        }
        ...
        if (!weaponChanged)
        {
            ...
            if (heldShoot > 0 && heldShoot < phantom.ownedProjectileCounts.Length)
            {
                phantom.ownedProjectileCounts[heldShoot] = oldProjCount;
            }
        }
        ...
        try
        {
            ...
            //TODO: why arkhalis is checked in particular?
            if (!p.active || (p.ModProjectile == null && p.type != Terraria.ID.ProjectileID.Arkhalis)) return;
    ...
    public void Initialize(float initialAngle) => currentOrbitAngle = initialAngle;
