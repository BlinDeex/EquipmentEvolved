...
public class UnidentifiedRareCharm : ModItem
{
    ...
    {
        tooltips[0].OverrideColor = CharmBalance.GetCharmColor(CharmRarity.Rare);

        tooltips.Add(new TooltipLine(Mod, "IdentifyDesc", "Right-click to identify.\nContains a random Rare charm.")
        {
    ...
    public override void RightClick(Player player)
    {
        ...
        if (Main.item[index].ModItem is Charm charm)
        {
            ...
            CharmsManager.InitializeCraftedCharm(charm, CharmRarity.Rare, randomType, player);
        }
    }
    ...
    {
        CreateRecipe()
            .AddIngredient<CommonCharmDust>(10)
    ...
    {
        Charm.DrawCharmShaderStatic(spriteBatch, position, 0f, scale, Main.UIScaleMatrix, GetShiftingShape(), CharmRarity.Rare);
        return false; 
    }
    ...
    public override bool PreDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI)
    {
        ...
        Charm.DrawCharmShaderStatic(spriteBatch, drawPos, rotation, scale, Main.GameViewMatrix.TransformationMatrix, GetShiftingShape(), CharmRarity.Rare);
        return false;
    }
