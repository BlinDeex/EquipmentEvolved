...
public class CharmsModSystem : ModSystem
{
    public override void Load()
    {
        ...
        {
            if (item.type != ModContent.ItemType<Charm>()) return orig.Invoke(context, item, stack, noStack, text);

            if (!Main.showItemText) return -1;

            if (item.Name == null || !item.active) return -1;
        ...
        IL_Main.MouseTextInner += il => //TODO: Detour/IL ChatManager.DrawColorCodedStringWithShadow directly so I can apply shaders in the future
        {
            ...
            c.EmitDelegate<Func<Color, Color>>(color =>
            {
                ...
                if (TryGetMouseHoverItem(out Item item) && item.ModItem is Charm charm) targetColor = charm.CharmColor;
