...
{
    /// <summary>
    /// Converts the CURRENT hit into True Damage (for ModifyHitNPC).
    /// </summary>
    public static void DealTrueDamage(NPC target, ref NPC.HitModifiers modifiers, float trueDamageDealt, bool disableNormalDamage)
    {
        ...
        if (disableNormalDamage)
        {
            ...
            modifiers.FinalDamage.Flat = dmgInt;
        ...
        if (Main.netMode != NetmodeID.Server)
        {
            ...
            {
                SendTrueDamagePacket(target, dmgInt);
            }
        }
    }
    ...
    public static void InflictTrueDamage(NPC target, float damage)
    {
        ...
        // 1. Deal the Damage (SimpleStrike ignores defense)
        // noOverhead: true -> Hides vanilla text so we can show ours
        //target.SimpleStrikeNPC(dmgInt, 0, false, 0f, DamageClass.Default, false, 0, true);
        ...
        // 2. Visuals (Same logic as DealTrueDamage)
        if (Main.netMode != NetmodeID.Server)
        {
            CombatText.NewText(UtilMethods.GetCombatTextRect(target), Color.WhiteSmoke, dmgInt, true, true);
    ...
    public static void SpawnPerceptiveText(Vector2 position, int damage, int tier)
    {
        ...
        if (textIndex > -1 && textIndex < Main.combatText.Length)
        {
            Main.combatText[textIndex].scale = 1.4f + (tier * 0.15f);
        }
    }
}