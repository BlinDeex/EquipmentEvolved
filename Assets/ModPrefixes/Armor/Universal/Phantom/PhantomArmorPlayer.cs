...
public class PhantomArmorPlayer : ModPlayer
{
    ...
    public override void PostUpdateMiscEffects()
    {
        ...
        if (activePhantoms.Count == 0)
        {
            ...
            if (buffIndex >= 0) Player.buffTime[buffIndex] -= PrefixBalance.PHANTOM_RECOVERY_SPEED;
        }
    }
    ...
    private void SoulBurnSound()
    {
        ...
        if (intensity >= START_THRESHOLD)
        {
            if (!SoundEngine.TryGetActiveSound(soulBurnSlot, out ActiveSound activeSound))
            {
                ...
                if (SoundEngine.TryGetActiveSound(soulBurnSlot, out activeSound)) activeSound.Volume = 0f; // workaround by keeping volume at 1 then immediately muting it didn't work
            }
            else
            {
        ...
        {
            if (SoundEngine.TryGetActiveSound(soulBurnSlot, out ActiveSound activeSound)) activeSound.Stop();
        }
    }
    ...
    public void SoulBurnEffect()
    {
        ...
        if (buffIndex >= 0)
        {
            ...
            if (!Filters.Scene["EquipmentEvolved:SoulBurn"].IsActive()) Filters.Scene.Activate("EquipmentEvolved:SoulBurn");

            Filters.Scene["EquipmentEvolved:SoulBurn"].GetShader().UseIntensity(intensity);
        }
        else
        {
            if (Filters.Scene["EquipmentEvolved:SoulBurn"].IsActive()) Filters.Scene["EquipmentEvolved:SoulBurn"].Deactivate();
        }
    }
}