...
public class PhantomSoundDistortion : ModSystem
{
    ...
    {
        if (!style.HasValue || Main.gameMenu || Main.LocalPlayer == null || !Main.LocalPlayer.active) return orig(ref style, position, updateCallback);

        int buffIndex = Main.LocalPlayer.FindBuffIndex(ModContent.BuffType<SoulBurnBuff>());

        if (buffIndex < 0) return orig(ref style, position, updateCallback);
