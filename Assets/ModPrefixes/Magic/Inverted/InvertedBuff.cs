...
public class InvertedBuff : ModBuff
{
    ...
    {
        InvertedModPlayer prefixPlayer = player.GetModPlayer<InvertedModPlayer>();
        ...
        if (player.whoAmI == Main.myPlayer)
        {
            ...
            if (dice > PrefixBalance.INVERTED_MANA_SURGE_CHANCE_TO_DAMAGE) player.Hurt(MiscStuff.GetManaSurgeDeath(Main.LocalPlayer.name), PrefixBalance.INVERTED_MANA_SURGE_DAMAGE, 0, quiet: false);
        }
    ...
    {
        InvertedModPlayer prefixPlayer = Main.LocalPlayer.GetModPlayer<InvertedModPlayer>();
